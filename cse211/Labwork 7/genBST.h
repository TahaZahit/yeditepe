...
public:
	...
	void printsmallerthan(T el);
	bool isbalanced();


private:
	void nth(BSTNode<T> *p, int n, vector<T> & s, bool *flag);
	void smaller(BSTNode<T> *p, T el,bool * flag);
	int checkBalance(BSTNode<T> * currentNode);
...
template<class T>
inline T BST<T>::getnthelement(int n)
{
	bool flag = true;
	vector<T> v;
	nth(root, n, v, &flag);
...
template<class T>
void BST<T>::nth(BSTNode<T>* p, int n, vector<T> & s, bool * flag)
{
	if (p != 0 && *flag) {
		nth(p->left, n, s, flag);
		if (*flag)s.push_back(p->el);
		if (s.size() == n) *flag = false;
		if (*flag)nth(p->right, n, s, flag);
	}
}

template<class T>
void BST<T>::printsmallerthan(T el)
{
	bool flag = true;
	smaller(root, el,&flag);
	cout << endl;
}


template<class T>
void BST<T>::smaller(BSTNode<T> *p, T el, bool * flag) {
	if (p != 0&& *flag) {
		smaller(p->left, el,flag);
		if (p->el < el) cout << p->el;
		else *flag = false;
		if(*flag)smaller(p->right, el,flag);
	}
}
template<class T>
int BST<T>::checkBalance(BSTNode<T> * currentNode)
{
	if (currentNode == 0) return 0;
	int leftHight = checkBalance(currentNode->left);
	if (leftHight == -1) return -1;
	int rightHight = checkBalance(currentNode->right);
	if (rightHight == -1) return -1;
	int absolute = (leftHight - rightHight) < 0 ? (rightHight - leftHight) : leftHight - rightHight;
